...
import { PLANNER_INTRO } from '../utils/plannerCopy';

/** Giải thích cho người dùng tính năng này làm gì, và quan trọng hơn là KHÔNG làm gì. */
function PlannerIntroPanel({ collapsed, onToggle }) {
  if (collapsed) {
    return (
      <button type="button" className="wp-intro__reopen" onClick={onToggle}>
        <ChevronRightIcon size={16} />
        {PLANNER_INTRO.expandLabel}
      </button>
    );
  }
    ...
    <section className="wp-card wp-intro">
      <div className="wp-intro__head">
        <span className="wp-intro__eyebrow">
          <SparkleIcon size={16} />
          {PLANNER_INTRO.eyebrow}
        </span>
        <h2 className="wp-intro__title">{PLANNER_INTRO.title}</h2>
        <p className="wp-intro__lead">{PLANNER_INTRO.lead}</p>
      </div>

      <ol className="wp-steps">
        {PLANNER_INTRO.steps.map((step) => (
      ...
      <div className="wp-intro__grid">
        <div className="wp-intro__block wp-intro__block--reads">
          <h3 className="wp-intro__block-title">{PLANNER_INTRO.readsTitle}</h3>
          <ul>
            {PLANNER_INTRO.reads.map((item) => (
              <li key={item}>{item}</li>
            ))}
          </ul>
        </div>
        <div className="wp-intro__block wp-intro__block--produces">
          <h3 className="wp-intro__block-title">{PLANNER_INTRO.producesTitle}</h3>
          <ul>
            {PLANNER_INTRO.produces.map((item) => (
              <li key={item}>{item}</li>
            ))}
          </ul>
        </div>
        <div className="wp-intro__block wp-intro__block--not">
          <h3 className="wp-intro__block-title">{PLANNER_INTRO.notTitle}</h3>
          <ul>
            {PLANNER_INTRO.not.map((item) => (
              <li key={item}>{item}</li>
            ))}
          </ul>
        </div>
      ...
      <div className="wp-intro__notes">
        <p className="wp-intro__note">
          <ClockIcon size={14} />
          {PLANNER_INTRO.durationNote}
        </p>
        <p className="wp-intro__note">{PLANNER_INTRO.draftNote}</p>
      </div>
      ...
      <button type="button" className="wp-intro__collapse" onClick={onToggle}>
        ...
        {PLANNER_INTRO.collapseLabel}
      </button>
    </section>
  );
}

export default PlannerIntroPanel;
